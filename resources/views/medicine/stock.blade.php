        ...
        <tbody>
            ...
            @foreach ($medicines as $item)
                <tr>
                    ...
                    <td>{{ $item['name'] }}</td>
                    <td
                        style="{{ $item['stock'] <= 3 ? 'background: red; color: white;' : ($item['stock'] > 10 ? 'background: green; color: white;' : ($item['stock'] > 15 ? 'background: yellow; color: black;' : 'background: none; color: black;')) }}">
                        {{ $item['stock'] }}
                    </td>
                    {{-- <td style="{{ $item->stock <= 3 ? 'background: red; color: white;' : 'background: none; color: black;' }}">{{ $item->stock }}</td> --}}
                    <td class="d-flex justify-content-center">
                        <div onclick="edit({{ $item['id'] }})" class="btn btn-primary me-3" style="cursor: pointer"><i
                                data-feather="plus-square"></i> Stock</div>
                    </td>
                </tr>
            @endforeach
        </tbody>
